...
let histogram_of_prog (p : prog) : histogram * int =
  ...
  let h_ins (_, ops) =
    incr size;
    List.iter h_opnd ops
  in

  let h_elem { lbl; global; asm } =
    match asm with Text insns -> List.iter h_ins insns | Data _ -> ()
  in
...
let memop_of_prog (p : prog) : int =
  ...
  let m_opcode = function
    ...
    | Movq | Leaq | Incq | Decq | Negq | Notq | Addq | Subq | Imulq | Xorq | Orq
    | Andq | Shlq | Sarq | Shrq | Jmp | J _ | Cmpq | Set _ | Callq | Retq ->
        ()
  in
  let h_ins (oc, ops) =
    m_opcode oc;
...
let string_of_histogram (h : histogram) : string =
  String.concat "| "
    (List.mapi
       (fun i r -> Printf.sprintf "%s %d " (string_of_reg r) h.(i))
       regs)
