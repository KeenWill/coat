...
type elt = 
  ...
  | E of Ll.uid * Ll.insn      (* hoisted entry block instructions *)
...
*)
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp Ast.node) : Ll.ty * Ll.operand * stream =
  match exp.elt with
  ...
  | Ast.Uop (uop, e) ->
    ...
    let cmp_uop op = function
      ...
      | Ast.Bitnot -> Ll.Binop (Xor, I64, op, i64_op_of_int (-1)) in
  ...
  | Ast.NewArrInit (elt_ty, e1, id, e2) ->    
    ...
    let arr_ty, arr_op, alloc_code = oat_alloc_array tc elt_ty size_op in
    let for_loop = Ast.(no_loc @@ For ([(id, no_loc (CInt 0L))],
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:Ast.exp Ast.node) : Ll.ty * Ll.operand * stream =
  match e.elt with
  | Ast.Id x ->
...
and cmp_call (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp Ast.node) (es:Ast.exp Ast.node list) : Ll.ty * Ll.operand * stream =
...
and cmp_exp_as (tc : TypeCtxt.t) (c:Ctxt.t) (e:Ast.exp Ast.node) (t:Ll.ty) : Ll.operand * stream =
 ...
 *)
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt Ast.node) : Ctxt.t * stream =
  ...
  | Ast.For (inits, guard, after, body) ->
     ...
     let guard = match guard with Some e -> e | None -> Ast.no_loc (Ast.CBool true) in
...
let cmp_global_ctxt (tc : TypeCtxt.t) (c:Ctxt.t) (p:Ast.prog) : Ctxt.t =
  let gexp_ty (c: Ctxt.t) : Ast.exp -> Ll.ty = function
    ...
    | x -> failwith ( "bad global initializer: " ^ (Astlib.string_of_exp (Ast.no_loc x)))
  in
  List.fold_left (fun c -> function
...
(* Compile a function declaration in global context c. Return the LLVMlite cfg
   ...
   in the function.
 *)
let cmp_fdecl (tc : TypeCtxt.t) (c:Ctxt.t) (f:Ast.fdecl Ast.node) : Ll.fdecl * (Ll.gid * Ll.gdecl) list =
...
*)
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp Ast.node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  match e.elt with
  ...
  | CArr (u, cs) ->
    ...
    let arr_i : Ll.ginit = GStruct [ I64, GInt (Int64.of_int len); Array(len, ll_u), GArray elts ] in
...
let tctxt_to_tdecls (c: TypeCtxt.t) =
  List.map (fun (i, l) -> i, Ll.Struct (List.map (fun (f: Ast.field) -> cmp_ty c f.ftyp) l)) c
