...
let parse_ll_file filename =
  let program =
    read_file filename |> Lexing.from_string |> Llparser.prog Lllexer.token
  in
  program
...
let run_program (args : string) (executable : string) (tmp_out : string) :
    string =
