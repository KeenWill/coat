...
let string_of_unop = function Neg -> "-" | Lognot -> "!" | Bitnot -> "~"
...
and print_ret_ty_aux fmt t =
  let pps = pp_print_string fmt in
  match t with RetVoid -> pps "void" | RetVal t -> print_ty_aux fmt t
...
and print_exp_aux level fmt e =
  ...
  | CStruct (t, l) ->
      ...
      List.iter
        (fun s ->
          print_cfield_aux this_level fmt s;
          pps "; ")
        l;
      pps "}";
      pp_close_box fmt ()
...
and print_exps_aux l r fmt es =
  ...
  print_list_aux fmt
    (fun () ->
      pps ",";
      pp_print_space fmt ())
    (fun fmt e -> print_exp_aux 0 fmt e)
    es;
  pp_close_box fmt ();
  pps r
  ...
  | For (decls, eo, so, body) ->
      ...
      print_list_aux fmt
        (fun () ->
          pps ",";
          ppsp ())
        (print_vdecl_aux "") decls;
      pps ";";
      ppsp ();
      (match eo with None -> () | Some e -> print_exp_aux 0 fmt e);
      pps ";";
      ppsp ();
...
let print_gdecl_aux fmt (gd : Ast.gdecl) =
  ...
  let ppsp = pp_print_space fmt in
  pp_open_hbox fmt ();
  pps @@ Printf.sprintf "global %s =" gd.name;
  ppsp ();
  print_exp_aux 0 fmt gd.init;
  pps ";";
  pp_close_box fmt ()
...
let print_tdecl_aux fmt ((id, l) : Ast.tdecl) =
  ...
  let ppsp = pp_print_space fmt in
  pp_open_hbox fmt ();
  pps @@ Printf.sprintf "struct %s =" id;
  ppsp ();
  pps "{";
  List.iter
    (fun s ->
...
let ml_string_of_option (f : 'a -> string) (o : 'a option) : string =
  match o with None -> sp "None" | Some x -> sp "Some (%s)" (f x)
...
and ml_string_of_ret_ty r =
  match r with RetVoid -> "TVoid" | RetVal t -> ml_string_of_ty t
...
let rec ml_string_of_exp_aux (e : exp) : string =
  ...
  | CStr s -> sp "CStr %S" s
  | CArr (t, cs) ->
      sp "CArr (%s,%s)" (ml_string_of_ty t)
        (ml_string_of_list ml_string_of_exp cs)
  | CStruct (id, l) ->
      sp "CStruct (%s, %s)" id (ml_string_of_list ml_string_of_field l)
  | Id id -> sp "Id %s" (ml_string_of_id id)
  | Index (e, i) ->
      sp "Index (%s, %s)" (ml_string_of_exp e) (ml_string_of_exp i)
  | Call (e, exps) ->
      sp "Call (%s, %s)" (ml_string_of_exp e)
        (ml_string_of_list ml_string_of_exp exps)
  | NewArr (t, e1) ->
      sp "NewArr (%s,%s)" (ml_string_of_ty t) (ml_string_of_exp e1)
  | NewArrInit (t, e1, u, e2) ->
      sp "NewArrInit (%s,%s,%s,%s)" (ml_string_of_ty t) (ml_string_of_exp e1)
        (ml_string_of_id u) (ml_string_of_exp e2)
  | Proj (exp, id) ->
      sp "Proj (%s,%s)" (ml_string_of_exp exp) (ml_string_of_id id)
  | Bop (b, e1, e2) ->
      sp "Bop (%s,%s,%s)" (ml_string_of_binop b) (ml_string_of_exp e1)
        (ml_string_of_exp e2)
...
let ml_string_of_vdecl_aux ((id, init) : vdecl) : string =
  sp "(%s, %s)" (ml_string_of_id id) (ml_string_of_exp init)
...
let rec ml_string_of_stmt_aux (s : stmt) : string =
  ...
  | SCall (exp, exps) ->
      sp "SCall (%s, %s)" (ml_string_of_exp exp)
        (ml_string_of_list ml_string_of_exp exps)
  | If (e, b1, b2) ->
      sp "If (%s,%s,%s)" (ml_string_of_exp e) (ml_string_of_block b1)
        (ml_string_of_block b2)
  | Cast (r, id, exp, null, notnull) ->
      sp "Cast (%s,%s,%s,%s,%s)" (ml_string_of_reft r) id (ml_string_of_exp exp)
        (ml_string_of_block null)
        (ml_string_of_block notnull)
  | For (d, e, s, b) ->
      sp "For (%s,%s,%s,%s)"
        (ml_string_of_list ml_string_of_vdecl_aux d)
        (ml_string_of_option ml_string_of_exp e)
        (ml_string_of_option ml_string_of_stmt s)
        (ml_string_of_block b)
  | While (e, b) ->
      sp "While (%s,%s)" (ml_string_of_exp e) (ml_string_of_block b)
...
let rec ml_string_of_fdecl_aux (f : fdecl) : string =
  sp "{ rtyp = %s; name = %s; args = %s; body = %s }"
    (ml_string_of_ret_ty f.frtyp)
    (ml_string_of_id f.fname) (ml_string_of_args f.args)
    (ml_string_of_block f.body)
...
let ml_string_of_gdecl_aux (gd : gdecl) : string =
  sp "{ name = %s; init = %s }" (ml_string_of_id gd.name)
    (ml_string_of_exp gd.init)
...
let ml_string_of_field { fieldName; ftyp } : string =
  sp "{ fname = %s; typ = %s }"
    (ml_string_of_id fieldName)
    (ml_string_of_ty ftyp)

let ml_string_of_tdecl_aux (id, fs) : string =
  sp "(id = %s, fs = (%s))" (ml_string_of_id id)
    (ml_string_of_list ml_string_of_field fs)
...
let ml_string_of_prog : prog -> string = ml_string_of_list ml_string_of_decl
