...
let insn_flow ((u, i) : uid * insn) (d : fact) : fact =
  ...
  | Call (_, op, _) ->
      UidM.add u SymPtr.MayAlias
        (match op with Id u -> UidM.add u SymPtr.MayAlias d | _ -> d)
  | Bitcast (_, op, _) ->
      UidM.add u SymPtr.MayAlias
        (match op with Id u -> UidM.add u SymPtr.MayAlias d | _ -> d)
  | Gep (_, op, _) ->
      UidM.add u SymPtr.MayAlias
        (match op with Id u -> UidM.add u SymPtr.MayAlias d | _ -> d)
  | Store (ty, op, _) -> (
      match (ty, op) with Ptr _, Id u -> UidM.add u SymPtr.MayAlias d | _ -> d)
...
module Fact = struct
  ...
  *)
  ...
  let merger_aux (u : uid) (mo1 : SymPtr.t option) (mo2 : SymPtr.t option) :
      SymPtr.t option =
    match (mo1, mo2) with
    | None, None -> None
    | Some _, None -> mo1
