...
type bid = GlobId of gid | HeapId of mid | StckId of fid | NullId
...
type sval = VUndef | VInt of int64 | VPtr of ptr

(* Memory values *)
type mtree = MWord of sval | MStr of string | MNode of mval
...
let mval_of_gdecl (gd : gdecl) : mval =
  let rec mtree_of_gdecl : gdecl -> mtree = function
    ...
    | _, GString s -> MStr s
    | _, GArray gs | _, GStruct gs -> MNode (List.map mtree_of_gdecl gs)
  in
  [ mtree_of_gdecl gd ]
...
let string_of_ptr (t, b, i) =
  pp "%s %s %s" (string_of_ty t) (string_of_bid b) (mapcat ", " string_of_int i)
...
let interp_cnd (c : cnd) (v1 : sval) (v2 : sval) =
  ...
  | VInt i, VInt j, _ -> VInt (if f i j then 1L else 0L)
...
let rec interp_operand (nt : tid -> ty) (locs : locals) (ty : ty) (o : operand)
    : sval =
  match (ty, o) with
  | I64, Const i -> VInt i
...
let update f k v k' = if k = k' then v else f k'
...
let replace_nth (l : 'a list) (n : int) (a : 'a) : 'a list =
  let rec loop acc n = function
    | [] -> raise Not_found
    | a' :: l' ->
        if n = 0 then List.rev_append acc (a :: l')
...
let rec load_idxs (m : mval) (idxs : idx list) : mtree =
  ...
  | i :: idxs' -> (
      ...
      if len <= i || i < 0 then raise OOBIndexDeref
      else
        match (idxs', List.nth m i) with
        | [], mt -> mt
        | [ 0 ], MStr s ->
            MStr s (* [n x i8]* %p and gep [n x i8]* %p, 0, 0 alias *)
...
let load_bid (c : config) (bid : bid) : mval =
  ...
  | HeapId mid -> (
      try List.assoc mid c.heap with Not_found -> raise UseAfterFree)
  | GlobId gid -> (
      try List.assoc gid c.globals
      with Not_found -> failwith "Load: bogus gid")
  | StckId fid -> (
      try List.assoc fid c.stack with Not_found -> raise UseAfterFree)
...
let effective_tag (nt : tid -> ty) ((tag, _, idxs) : ptr) : ty =
  let rec loop tag idxs =
    ...
    | Struct ts, i :: idxs' ->
        if List.length ts <= i then
          failwith "effective_tag: index oob of struct"
  ...
  try List.tl idxs
  with Failure _ ->
    failwith "effective_tag: invalid pointer missing first index"
...
let gep_ptr (nt : tid -> ty) (ot : ty) (p : ptr) (idxs' : idx list) : sval =
  if not (legal_gep nt ot @@ effective_tag nt p) then VUndef
  else
    match p with
    | t, NullId, idxs -> VUndef
    | t, bid, idxs -> VPtr (t, bid, gep_idxs idxs idxs')
...
let interp_prog { tdecls; gdecls; fdecls } (args : string list) : sval =
  ...
  let runtime_fns = [ "ll_puts"; "ll_strcat"; "ll_ltoa" ] in

  (* External function implementation *)
  let runtime_call (t : ty) (fn : gid) (args : sval list) (c : config) :
      config * sval =
    let load_strptr p =
      match load_ptr c p with
      | MStr s -> s
  ...
  let rec interp_cfg ((k, blocks) : cfg) (locs : locals) (c : config) :
      config * sval =
    ...
    | (u, Load (Ptr t, o)) :: insns, _ ->
        let mt =
          match interp_op locs (Ptr t) o with
          | VPtr p ->
    ...
    | (u, Call (t, ofn, ato)) :: insns, _ ->
        ...
        let g =
          match interp_op locs ft ofn with
          | VPtr (_, GlobId g, [ 0 ]) -> g
    ...
    | (u, Gep (Ptr t, o, os)) :: insns, _ ->
        ...
        let locs' = update locs u v' in
        interp_cfg ({ k with insns }, blocks) locs' c
    | [], (_, Ret (_, None)) -> ({ c with stack = List.tl c.stack }, VUndef)
    | [], (_, Ret (t, Some o)) ->
    ...
    | [], (_, Cbr (o, l1, l2)) ->
        ...
        interp_cfg (k', blocks) locs c
    | (u, i) :: _, _ ->
        failwith @@ "interp_cfg: invalid instruction \"" ^ string_of_insn i
        ^ "\" at %" ^ u
  and interp_call (ty : ty) (fn : gid) (args : sval list) (c : config) :
      config * sval =
    if List.mem fn runtime_fns then runtime_call ty fn args c
    else
      let { f_param; f_cfg } =
        try List.assoc fn fdecls
  ...
  let _, r =
    interp_call I64 "main" [ argc; argv ] { globals; heap; stack = [] }
  in
  r
