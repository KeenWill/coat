...
let rec string_of_ginit : ginit -> string = function
  ...
  | GBitcast (t1, g, t2) ->
      pp "bitcast (%s %s to %s)" (sot t1) (string_of_ginit g) (sot t2)
...
let string_of_named_edecl ((g, t) : gid * ty) : string =
  match t with
  | Fun (ts, rt) ->
      pp "declare %s @%s(%s)" (string_of_ty rt) g (mapcat ", " string_of_ty ts)
