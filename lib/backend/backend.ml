...
module Alloc = struct
  ...
  let of_block (f : Ll.uid -> loc) (g : Ll.gid -> X86.lbl)
      (live_in : Ll.uid -> UidSet.t) (b : Ll.block) : fbody =
    List.map
      (fun (u, i) ->
        (* Uncomment this to enable verbose debugging output... *)
        Platform.verb
        @@ Printf.sprintf "  * of_block: %s live_in = %s\n" u
             (UidSet.to_string (live_in u));
  ...
  let of_cfg (f : Ll.uid -> loc) (g : Ll.gid -> X86.lbl)
      (live_in : Ll.uid -> UidSet.t) ((e, bs) : Ll.cfg) : fbody =
    List.(
...
let emit_mov (src : X86.operand) (dst : X86.operand) : x86stream =
  ...
  | Imm (Lbl l), _ ->
      lift
        X86.Asm.[ (Leaq, [ Ind3 (Lbl l, Rip); ~%Rax ]); (Movq, [ ~%Rax; dst ]) ]
...
let compile_pmov live (ol : (Alloc.loc * Ll.ty * Alloc.operand) list) :
  ...
  let rec loop outstream ol =
    ...
    | (x, _, o) :: ol', [] ->
        let os =
          loop (outstream >:: I X86.Asm.(Pushq, [ compile_operand o ])) ol'
        in
...
let compile_call live (fo : Alloc.operand) (os : (Ll.ty * Alloc.operand) list) :
    x86stream =
  let oreg, ostk, _ =
    List.fold_left
      (fun (oreg, ostk, i) (t, o) ->
...
let compile_getelementptr tdecls (t : Ll.ty) (o : Alloc.operand)
  ...
  let rec loop (ty : Ll.ty) path (code : x86stream) =
    ...
    | Struct ts, Alloc.Const n :: rest ->
        ...
        loop u rest @@ (code >:: I X86.Asm.(Addq, [ ~$offset; ~%Rax ]))
  ...
  let rec loop (af : Alloc.fbody) (outstream : x86stream) : x86stream =
    ...
    | (Binop (LReg r, bop, _, o, o'), _) :: rest
      when Loc (LReg r) = o'
        ...
        loop rest @@ (outstream >:: I X86.Asm.(cb bop, [ co o; ~%r ]))
    ...
    | (Binop (x, b, _, o, o'), _) :: rest ->
        ...
        @@ (outstream
           ...
           >@ lift
                X86.Asm.
                  [ (cb b, [ co o'; ~%Rax ]); (Movq, [ ~%Rax; co (Loc x) ]) ])
    | (Alloca (x, at), _) :: rest ->
        loop rest
        @@ (outstream
           >@ lift
    ...
    | (Load (LReg x, _, Loc (LReg src)), _) :: rest ->
        loop rest @@ (outstream >:: I X86.Asm.(Movq, [ Ind2 src; ~%x ]))
    | (Load (x, _, src), _) :: rest ->
        loop rest
        @@ (outstream
    ...
    | (Store (_, Loc (LReg src), Loc (LReg dst)), _) :: rest ->
        loop rest @@ (outstream >:: I X86.Asm.(Movq, [ ~%src; Ind2 dst ]))
    ...
    | (Call (x, t, fo, os), live) :: rest ->
        ...
        let fptr_op, init_fp, restore_fp =
          ...
          | Loc (LReg (X86.Rdi | Rsi | Rdx | Rcx | R08 | R09)) ->
              ( Loc (LReg X86.R15),
                [ I X86.Asm.(Pushq, [ ~%X86.R15 ]) ]
                >@ emit_mov (co fo) (Reg X86.R15),
        ...
        @@ (outstream
           ...
           >@ lift
                X86.Asm.
                  [ (Movq, [ ~%Rbp; ~%Rsp ]); (Popq, [ ~%Rbp ]); (Retq, []) ])
    | (Br (LLbl l), _) :: rest ->
        loop rest @@ (outstream >:: I X86.Asm.(Jmp, [ ~$$l ]))
    | (Cbr (Const i, LLbl l1, LLbl l2), _) :: rest ->
        loop rest
        @@ (outstream
           >::
           if i <> 0L then I X86.Asm.(Jmp, [ ~$$l1 ])
...
let fold_fdecl (f_param : 'a -> Ll.uid * Ll.ty -> 'a)
    (f_lbl : 'a -> Ll.lbl -> 'a) (f_insn : 'a -> Ll.uid * Ll.insn -> 'a)
    (f_term : 'a -> Ll.uid * Ll.terminator -> 'a) (init : 'a) (f : Ll.fdecl) :
    'a =
...
module Graph = struct
  ...
  let put_adj (u : Ll.uid) (adj : UidS.t) (g : t) : t =
    if 0 = UidS.cardinal adj then
      UidM.update_or UidSet.empty (fun (s : UidSet.t) -> s) u g
    else UidS.fold (fun uid acc -> add_edge acc uid u) adj g

  let put_adj_list (u : Ll.uid) (adj : Ll.uid list) (g : t) : t =
    if 0 = List.length adj then
      UidM.update_or UidSet.empty (fun (s : UidSet.t) -> s) u g
    else List.fold_right (fun uid acc -> add_edge acc uid u) adj g

  let cut_graph (u : Ll.uid) (g : t) : t =
    UidM.remove u g |> UidM.map (UidS.remove u)

  let get_neighbors (u : Ll.uid) (g : t) : UidSet.t =
    UidM.find_or UidSet.empty g u

  let get_degree (u : Ll.uid) (g : t) : int = UidS.cardinal @@ get_neighbors u g

  let degree_less (i : int) (u : Ll.uid) (g : t) : bool = get_degree u g < i

  let filter_nodes (f : Ll.uid -> t -> bool) (g : t) : UidS.t =
    UidM.fold
      (fun (key : Ll.uid) _ (acc : UidS.t) ->
        if f key g then UidS.add key acc else acc)
      g UidS.empty
  ...
  let rec kempe_stack (k : int) (g : t) : Ll.uid list =
    let find_degree (f : int -> int -> bool) =
      let b, best =
        UidM.fold
          (fun u adj (b, best) ->
            if f (UidS.cardinal adj) (UidS.cardinal best) then (u, adj)
            else (b, best))
          g ("", UidS.empty)
      in
    ...
    match (UidS.choose_opt color_set, max_degree ()) with
    | None, None -> []
    | None, Some uid | Some uid, _ -> uid :: kempe_stack k (cut_graph uid g)

  let color (g : t) (pal : LocSet.t) (precolored : lo) (prespilled : int) :
      lo * int =
    ...
    let color_node_opt (u : Ll.uid) (colored : lo) : Alloc.loc option =
      ...
      |> LocSet.diff pal |> LocSet.choose_opt
    in
    ...
    let color_aux (pc : lo) (uids : Ll.uid list) : lo =
      List.fold_left
        (fun acc u ->
          ...
          | None, Some c ->
              (u, c) :: acc (* first time coloring this node with a reg *)
          | Some _, _ -> acc
          (* this node is already colored so skip it *))
        pc uids
    in

    let colored = kempe_stack (LocSet.cardinal pal) g |> color_aux precolored in
    (colored, !n_spill)
end
...
let better_layout (f : Ll.fdecl) (live : liveness) : layout =
  ...
  let (g, args, precolored) : Graph.t * Ll.uid list * Graph.lo =
    fold_fdecl
      ...
      (fun (g, args, pc) (x, i) ->
        ...
        | Bitcast (_, op, _) -> (
            let live_out : UidSet.t =
              try live.live_out x with Not_found -> UidSet.empty
            in
            match op with
            | Id id ->
                let g = Graph.put_adj x live_out g |> Graph.remove_edge x id in
                (g, args, pc)
            | _ -> (Graph.put_adj x live_out g, args, pc))
        | Call (ty, op, tyopl) ->
            let n_arg = ref 0 in
            let alloc_arg_check () =
              match arg_loc !n_arg with Alloc.LReg Rcx -> spill () | x -> x
            in
            let alloc_arg () =
              ...
              List.fold_left
                (fun acc (ty, op) ->
                  ...
                  | Ll.Id uid ->
                      ...
                      let is_preassigned =
                        List.fold_left
                          (fun acc (k, v) -> optimal_choice <> v && acc)
                          true pc
                      in
                      print_string @@ string_of_bool is_preassigned;
                      if is_preassigned then (uid, alloc_arg ()) :: acc else acc
                  | _ -> acc)
                pc tyopl
            in
      ...
      (fun (g, args, pc) (x, term) ->
        ...
        | Ret (ty, op) -> (
            let rax_preassigned =
              List.fold_left
                (fun acc (k, v) -> Alloc.LReg Rax <> v && acc)
                true pc
            in
            print_string @@ string_of_bool rax_preassigned;
            match (op, rax_preassigned) with
  ...
  let (colorings, spilled) : (UidSet.elt * LocSet.elt) list * int =
    Graph.color g pal precolored !n_spill
  in
...
let compile_fdecl tdecls (g : Ll.gid) (f : Ll.fdecl) : x86stream =
  ...
  *)
  ...
  >@ lift X86.Asm.[ (Pushq, [ ~%Rbp ]); (Movq, [ ~%Rsp; ~%Rbp ]) ]
  >@ (if layout.spill_bytes <= 0 then []
     else lift X86.Asm.[ (Subq, [ ~$(layout.spill_bytes); ~%Rsp ]) ])
